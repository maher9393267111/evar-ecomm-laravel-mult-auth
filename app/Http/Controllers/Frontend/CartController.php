...
class CartController extends Controller
{
    ...
    public function cartStore(Request $request){
    	...
    	if($result){
            ...
            $response['message']="Item was added your cart";
    	}
        if($request->ajax()){
    ...
    public function cartUpdate(Request $request){
       ...
       if($request_quantity>$productQuantity){
        $message="Sorry we currently do not have item in stock";
        $response['status']=false;
    }   
    elseif($request_quantity<1){
        $message="You can not add product less then 1 quantity";
        $response['status']=false;
    }
    else{
    ...
    if($request->ajax()){
        $header= view('frontend.layouts.header')->render();
        $cart_list=view('frontend.layouts.cartlist')->render();
        $response['header']=$header;
    ...
    public function couponAdd(Request $request){
        ...
        if(!$coupon){
            return back()->with('error','Invalid Coupon code please try valid coupon code');
        }
        if($coupon){
